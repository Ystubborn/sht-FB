<template>
  <div>
    <BreadCrumb :data="[{a: '我要报案', b: {path: '/droit/insure'}}, {a: '客户财产损失申报'}]"></BreadCrumb>
    <div class="main-content w block-center safeinsure" v-if="!comm.IsNullOrEmpty(uiData)">
      ...
      <el-form :model="uiData" :rules="rules" ref="uiData" label-width="130px">
        ...
        <el-form-item label="损失照片：" prop="fendimage.id" is-required>
          <Upload :limit="5" v-model="uiData.fendimage"></Upload>
        </el-form-item>
        <el-form-item label="货物价值证明：">
          <Upload :limit="5" v-model="uiData.fcostprove"></Upload>
        </el-form-item>
        <el-form-item label="维修发票：">
          <Upload :limit="5" v-model="uiData.frepairinvoice"></Upload>
        </el-form-item>
      </el-form>
		...
		methods: {
			Save() {
				...
				t.$refs['uiData'].validate(valid => {
					...
					t.uiData.fsafetype.id = 'fsafe_type_02';
