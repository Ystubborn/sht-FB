<template>
  <div>
    ...
    <div class="main-content w block-center placeorder">
      <el-form  ref="uiData"  label-width="100px">
        <div class="title border-bottom-solid margin-bottom">基本信息</div>
        <div class="el-from-lable-vertical" label="服务类型：">
          <Radio type="border" v-model="uiData[2].itemCode"  :readonly="uiData[2].length>0"  readonly_msg="如需更改类型，请将服务信息的项目全部移除">
            <!-- <div v-for="item in uiData.liData" :key="item.itemCode" :value="item.itemCode">{{item}}</div> -->
			<div v-for="item in uiData[2]" :key="item.itemCode" :value="item.itemCode">
				{{item.itemName}}
			</div>
          </Radio>
        </div>    
        <el-form-item label="服务类目: " prop="fprofield.id">
          <Radio type="border" class="serviceicon" v-model="uiData[2].itemCode" :multiple="true">
            <div v-for="(item,index) in uiData[2]" :key="item.itemCode" :value="item.itemCode" v-show="index<5">
        ...
        <el-form-item label="客户：" is-required>
          <el-col :span="5">
            <el-form-item prop="fname">
              <el-input type="text"  placeholder="请输入客户姓名"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="5">
            <el-form-item prop="fphone" :rules="comm.rules.phone">
              <el-input type="text"  placeholder="请输入客户手机号"></el-input>
            </el-form-item>
          </el-col>
        </el-form-item>
        <el-form-item>
          ...
          <el-col :span="12">
            <el-form-item prop="fcusaddress">
              <el-input type="text" placeholder="请输入详细的地址" class="input-address margin-right"><i class="el-icon-location el-input__icon" slot="suffix"></i></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="5">
            <el-select class="i8">
        ...
        <div>
          ...
          <el-form-item label="是否到货：" is-required>
            <el-select placeholder="货物是否到达物流点" class="margin-right">
              <el-option></el-option>
            </el-select>
            <span class="color-red">如未到货可暂不填写物流信息。填写物流信息有助于服务商更准确地报价。</span>
          </el-form-item>
          <el-form-item label="物流公司：">
            <el-input type="text" placeholder="请输入物流公司名称" class="margin-right"></el-input>
            物流单号：　<el-input type="text" placeholder="请输入物流单号"></el-input>
          </el-form-item>
          <el-form-item label="提货地址：">
            <el-input type="text" placeholder="请输入提货详细地址" class="input-address margin-right"><i class="el-icon-location el-input__icon" slot="suffix"></i></el-input>
          </el-form-item>
          <el-form-item label="提货电话：">
            <el-input type="text" placeholder="请输入提货电话" class="margin-right"></el-input>
          ...
          <el-form-item label="代付运费：">
            <el-checkbox class="margin-right">需师傅代付运费，师傅报价包含此费用</el-checkbox>
            <em class="color-red">*</em>代付金额：　<el-input-number class="i8 margin-right" :min="0" :controls="false" :max="10000"></el-input-number>元
          </el-form-item>
        </div>
        ...
        <div class="service-item">
          ...
          <div>
            <el-input-number size="small" :min="0.001" :max="10000"></el-input-number>
          </div>
          <div>
            <el-select placeholder="可选房型" size="small">
              <el-option></el-option>
            </el-select>
          </div>
        ...
        <el-dialog title="添加项目" width="723px" center :close-on-click-modal="false">
          <Orderseritem @listenData="opChange"></Orderseritem>
        </el-dialog>
        <div class="service-item">
          <input type="button" value="添加项目" @click="opClick('showDialog')" />
        </div>
        <el-form-item label="图纸：" prop="fimage.id" is-required>
          <Upload :limit="50" class="margin-bottom"></Upload>
        </el-form-item>
        <div class="title border-bottom-solid margin-bottom">服务要求</div>
        <el-form-item label="服务日期" is-required>
          <el-date-picker type="date" placeholder="选择服务日期"></el-date-picker>
          <span class="color-red">(希望服务商在此日期上门服务)</span>
        </el-form-item>
        <el-form-item label="注意事项" prop="fcareful" is-required>
          <el-input :maxlength="200" class="textarea-big" :rows="4" type="textarea" placeholder="请填写关于客户以及物流备注，以及其他需要师傅注意的内容。（100字以内）"></el-input>
        </el-form-item>
        <el-form-item label="内部单号">
          <el-input placeholder="内部单号" class="input-medium"></el-input>
        </el-form-item>
...
export default	{
	...
	created(){
		...
		this.uiData=newArr
		console.log(this.uiData[1])
		// for(let j in newArr[0]){
		// 	console.log(newArr[0][j].itemCode)
		// }
	},
		methods: {
		// 	opChange(type, data) {
